...
namespace BLL
{
    public class BLL_Customer
    {
        ...
        // -- Thêm Tài Khoản --
        public bool themTK(string tdn, string mk, string hoTen, string email, int sdt, string diaChi)
        {
        ...
        public IEnumerable hienThiSPLoc(int minPrice, int maxPrice, int locTheo)
        {
            //if (locTheo == 1)
            //{
            //    return db.tbl_SanPhams.Where(sp => sp.so_luong_ton > 0 && sp.gia >= minPrice && sp.gia <= maxPrice).OrderBy(sp => sp.ten_sp).Select(sp => new { gia = String.Format(CultureInfo.InvariantCulture, "{0:#,#}", sp.gia), sp }).ToList();
            //}
            //else if (locTheo == 2)
            //{
            //    return db.tbl_SanPhams.Where(sp => sp.so_luong_ton > 0 && sp.gia >= minPrice && sp.gia <= maxPrice).OrderByDescending(sp => sp.ten_sp).Select(sp => new { gia = String.Format(CultureInfo.InvariantCulture, "{0:#,#}", sp.gia), sp }).ToList();
            //}
            //else 
            if (locTheo == 1)
            {
        ...
        public IEnumerable hienThiSPLocTheoDanhMuc(int minPrice, int maxPrice, int locTheo, string maLoai)
        {
            //if (locTheo == 1)
            //{
            //    return db.tbl_SanPhams.Where(sp => sp.so_luong_ton > 0 && sp.gia >= minPrice && sp.gia <= maxPrice && sp.ma_loai == maLoai).OrderBy(sp => sp.ten_sp).Select(sp => new { gia = String.Format(CultureInfo.InvariantCulture, "{0:#,#}", sp.gia), sp }).ToList();
            //}
            //else if (locTheo == 2)
            //{
            //    return db.tbl_SanPhams.Where(sp => sp.so_luong_ton > 0 && sp.gia >= minPrice && sp.gia <= maxPrice && sp.ma_loai == maLoai).OrderByDescending(sp => sp.ten_sp).Select(sp => new { gia = String.Format(CultureInfo.InvariantCulture, "{0:#,#}", sp.gia), sp }).ToList();
            //}
            //else 
            if (locTheo == 1)
            {
        ...
        public bool suaThongTinKH(string tdn_email, string hoTen, string diaChi, string email, int sdt)
        {
        ...
        public void datHang(DataTable gioHang, int maKH, string maSP, string diaChiGH, string sdtGH)
        {
